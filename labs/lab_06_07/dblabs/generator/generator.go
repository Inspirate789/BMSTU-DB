...
func generationStep1(dataPath string) ([]types.PartOfSpeech, []types.Model, []types.User, error) {
	...
	wg.Wait() // Step 1: partsOfSpeech + models || users

	wg.Add(1)
	go func() {
		defer wg.Done()

		users, tmpErr1 = GenerateUsers(dataPath, 5000)
		if tmpErr1 != nil {
			mx.Lock()
	...
	go func() {
		...
		models, tmpErr2 = GenerateModels(dataPath, 1000, partsOfSpeech)
		if tmpErr2 != nil {
			mx.Lock()
...
func generationStep2(dataPath string, partsOfSpeech []types.PartOfSpeech, models []types.Model) ([]types.Term, []types.Term, error) {
	...
	go func() {
		...
		termsRu, tmpErr1 = GenerateTerms(dataPath, 5000, models, "ru")
		if tmpErr1 != nil {
			mx.Lock()
	...
	go func() {
		...
		termsEn, tmpErr2 = GenerateTerms(dataPath, 5000, models, "en")
		if tmpErr2 != nil {
			mx.Lock()
	...
	go func() {
		...
		tmpErr3 = GenerateModelsAndPos(dataPath, 5000, models, partsOfSpeech)
		if tmpErr3 != nil {
			mx.Lock()
...
func generationStep3(dataPath string, termsRu []types.Term, termsEn []types.Term, users []types.User) error {
	...
	go func() {
		...
		contexts, tmpErr1 = GenerateContexts(dataPath, 1000, append(termsRu, termsEn...))
		if tmpErr1 != nil {
			mx.Lock()
		...
		go func() {
			...
			tmpErr1 = GenerateContextsAndTerms(dataPath, 5000, contexts, termsRu, "ru")
			if tmpErr1 != nil {
				mx.Lock()
		...
		go func() {
			...
			tmpErr2 = GenerateContextsAndTerms(dataPath, 5000, contexts, termsEn, "en")
			if tmpErr2 != nil {
				mx.Lock()
	...
	go func() {
		defer wg.Done()
		tmpErr3 = GenerateUsersAndTerms(dataPath, 5000, users, termsRu, "ru")
		if tmpErr3 != nil {
			mx.Lock()
	...
	go func() {
		defer wg.Done()
		tmpErr4 = GenerateUsersAndTerms(dataPath, 5000, users, termsEn, "en")
		if tmpErr4 != nil {
			mx.Lock()
...
// GenerateData parallel implementation
func GenerateData(dataPath string) error {
	partsOfSpeech, models, users, err := generationStep1(dataPath)
	if err != nil {
		return err
	}

	termsRu, termsEn, err := generationStep2(dataPath, partsOfSpeech, models)
	if err != nil {
		return err
	}

	err = generationStep3(dataPath, termsRu, termsEn, users)
	if err != nil {
		return err
	}
...
// GenerateData sequential implementation
//func GenerateData(dataPath string) error {
//	users, err := GenerateUsers(dataPath, 5000)
//	if err != nil {
//		return err
//	}
//
...
//	}
//
//	models, err := GenerateModels(dataPath, 1000, partsOfSpeech)
//	if err != nil {
//		return err
//	}
//
//	termsRu, err := GenerateTerms(dataPath, 5000, models, "ru")
//	if err != nil {
//		return err
//	}
//
//	termsEn, err := GenerateTerms(dataPath, 5000, models, "en")
//	if err != nil {
//		return err
//	}
//
//	contexts, err := GenerateContexts(dataPath, 1000, append(termsRu, termsEn...))
//	if err != nil {
//		return err
//	}
//
//	err = GenerateModelsAndPos(dataPath, 5000, models, partsOfSpeech)
//	if err != nil {
//		return err
//	}
//
//	err = GenerateContextsAndTerms(dataPath, 5000, contexts, termsRu, "ru")
//	if err != nil {
//		return err
//	}
//
//	err = GenerateContextsAndTerms(dataPath, 5000, contexts, termsEn, "en")
//	if err != nil {
//		return err
//	}
//
//	err = GenerateUsersAndTerms(dataPath, 5000, users, termsRu, "ru")
//	if err != nil {
//		return err
//	}
//
//	err = GenerateUsersAndTerms(dataPath, 5000, users, termsEn, "en")
//	if err != nil {
//		return err
//	}
//
